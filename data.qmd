...
missing values. The one difference is this one shows the missing values per column as a 
percentage of total rows. It reveals that only two columns have a significant amount of missing values. Those two columns are the the Mortgage_Status_By_Median_Value and the "Median_Contract_Rent". Overall the data set is highly reliable with minimal data loss.