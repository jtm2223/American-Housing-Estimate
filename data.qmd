...
state_data <- data |>
  ...
  summarise(
    ...
    top_counties = list(
      data |>
        filter(STATE_NAME == first(STATE_NAME)) |>
        arrange(desc(Median_value_for_units_with_a_mortgage)) |>
        select(NAME, Median_value_for_units_with_a_mortgage) |>
        head(5)
    )
  )
...
state_json <- state_data |>
  mutate(
    top_counties = lapply(top_counties, function(counties) {
      counties |> rename(county = NAME, 
                         value = Median_value_for_units_with_a_mortgage)
    })
  )

write_json(state_data, "d3Data/d3Data.json", pretty = TRUE)
```
